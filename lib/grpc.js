...
const { FunctionInvokerService, MessageBuilder, MessageHeaders, LivenessService } = require('@projectriff/function-proto');
...
function makeServer(fn) {
    ...
    server.addService(LivenessService, {
        probe: async (_, callback) => {
            if (typeof fn.$health !== 'function') {
                callback(null, true);
            }
            try {
                const healthy = !! await fn.$health();
